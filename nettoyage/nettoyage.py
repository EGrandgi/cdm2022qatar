...
def basic_cleaner(data):
    in_ = ["\'", '\s+', 'é', 'è', 'ë', 'ê', 'à', 'â', 'ä', 'î', 'ï', 'ô', 'ö', 'œ', 'æ', 'û', 'ü', 'ù', 'ú', 'ç', 'À', 'Â', 'É', 'È', 'Ê', 'Ë', 'Î', 'Ï', 'Ô', 'Ö', 'Ù', 'Û', 'Ü', 'Ç', 'Œ', 'Æ', 'Ã©', 'Ã¨', 'Ã¯', 'Ã´', 'Ã§', 'Ãª', 'Ã¹', 'Ã¦', 'Å', 'Ã«', 'Ã¼', 'Ã¢', 'â¬', 'Â©', 'Ã', 'Ã®', 'Ã¶', 'Ã»', 'Å', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Å', '\t', '\n', '\x92', '-', '/', 'Ãº']
    out_ = ['', ' ', 'e', 'e', 'e', 'e', 'a', 'a', 'a', 'i', 'i', 'o', 'o', 'oe', 'ae', 'u', 'u', 'u', 'u', 'c', 'a', 'a', 'e', 'e', 'e', 'e', 'i', 'i', 'o', 'o', 'u', 'u', 'u', 'c', 'oe', 'ae', 'e', 'e', 'i', 'o', 'c', 'e', 'u', 'ae', 'oe', 'e', 'u', 'a', 'e', 'c', 'a', 'i', 'o', 'u', 'oe', 'A', 'A', 'E', 'E', 'E', 'E', 'I', 'I', 'O', 'O', 'U', 'U', 'U', 'C', 'OE', '', '', '', '', '', 'u']      
    for j in range(len(in_)):
        data = [re.sub(in_[j], out_[j], sent) for sent in data]
    return data

def sent_to_words(sentences): #tokenizes each sentence into a list of words, removing punctuations and unnecessary characters
...
def clean_theme(df):
    for i in range(len(df)):
        theme = df.theme[i]
        in_ = ['é', 'è', 'ë', 'ê', 'à', 'â', 'ä', 'î', 'ï', 'ô', 'ö', 'œ', 'æ', 'û', 'ü', 'ù', 'ú', 'ç', 'À', 'Â', 'É', 'È', 'Ê', 'Ë', 'Î', 'Ï', 'Ô', 'Ö', 'Ù', 'Û', 'Ü', 'Ç', 'Œ', 'Æ', 'Ã©', 'Ã¨', 'Ã¯', 'Ã´', 'Ã§', 'Ãª', 'Ã¹', 'Ã¦', 'Å', 'Ã«', 'Ã¼', 'Ã¢', 'â¬', 'Â©', 'Ã', 'Ã®', 'Ã¶', 'Ã»', 'Å', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Ã', 'Å', '\t', '\n', '\x92', '-', '(', ')', '.', '/', '?', '!', 'Ãº']
        out_ = ['e', 'e', 'e', 'e', 'a', 'a', 'a', 'i', 'i', 'o', 'o', 'oe', 'ae', 'u', 'u', 'u', 'u', 'c', 'a', 'a', 'e', 'e', 'e', 'e', 'i', 'i', 'o', 'o', 'u', 'u', 'u', 'c', 'oe', 'ae', 'e', 'e', 'i', 'o', 'c', 'e', 'u', 'ae', 'oe', 'e', 'u', 'a', 'e', 'c', 'a', 'i', 'o', 'u', 'oe', 'A', 'A', 'E', 'E', 'E', 'E', 'I', 'I', 'O', 'O', 'U', 'U', 'U', 'C', 'OE', '', '', '', '', '', '', '', '', '', '', 'u']      
        for j in range(len(in_)):
...
filenames = glob.glob("C:\\Users\\Etudiant\\Desktop\\data\\*.json")
...
full_list = title_data_words_bigrams + subtitle_data_words_bigrams + content_data_words_bigrams
...
# =============================================================================
#                  Traiter les bigrams personnes et stades
# =============================================================================

path = 'C:\\Users\\Etudiant\\data\\'
df_stadiums = pd.DataFrame.from_csv(path + 'stadiums.csv', sep=';')
df_persons = pd.DataFrame.from_csv(path + 'persons.csv', sep=';')
...
df_persons = pd.DataFrame.from_csv(path + 'persons_list.csv', sep=';')
persons_list = df_persons['pers'].tolist()
df_articles_persons = pd.DataFrame(columns=['id_article', 'id_pers'])

df_stadiums = pd.DataFrame.from_csv(path + 'stadiums_list.csv', sep=';')
...
if __name__ == '__main__':
    ...
    filename = now[:10] + "_articles_clean.csv"
    df_all.to_csv(path + filename, encoding='utf-8', index=True, sep=';')
    
    df_articles_persons.to_csv(path + "articles_persons.csv", encoding='utf-8', index=True, sep=';')
    df_articles_stadiums.to_csv(path + "articles_stadiums.csv", encoding='utf-8', index=True, sep=';')
